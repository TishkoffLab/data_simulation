...
def write_genovcf(geno_dict,outname,seq_len):
...
if __name__ == "__main__":
    ...
    if(args.beta_type is None):
        ...
        beta = args.beta_type
    pgenos_byinds = run_pheno_simulation(int(args.samp_size),int(args.seq_len),int(args.L),int(args.num_individuals),args.out_name,beta)
    write_genovcf(pgenos_byinds,'{0}.pheno'.format(args.out_name),int(args.seq_len))

    genos_byinds = run_geno_simulation(int(args.samp_size),int(args.seq_len),int(args.L),int(args.num_individuals))
    write_genovcf(genos_byinds,'{0}.geno'.format(args.out_name),int(args.seq_len))

    # save_genotype_vcf(int(args.samp_size),int(args.seq_len),int(args.num_individuals),args.out_name)
